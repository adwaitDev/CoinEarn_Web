...
import com.adwait.aitrading.model.*;

import com.adwait.aitrading.repository.OrderItemRepository;
import com.adwait.aitrading.repository.OrderRepository;
...
public class OrderServiceImpl implements OrderService{
    ...
    private final WalletService walletService; // Changed to final
    private final OrderItemRepository orderItemRepository; // Changed to final
    ...
    public void cancelOrder(Long orderId) {
        ...
        if (order.getStatus() == OrderStatus.PENDING) {
            ...
            throw new IllegalStateException("Cannot cancel the order, since it is already processed or cancelled.");
        }
    }
    ...
    public Order sellAsset(Coin coin,double quantity, User user) throws Exception {
        ...
        Asset assetToSell = asset_service.findAssetByUserIdAndCoinId(user.getId(), coin.getId());

        if (assetToSell != null) {

            OrderItem orderItem = createOrderItem(coin,quantity, assetToSell.getBuy_price(), sellPrice);
            ...
            if (assetToSell.getQuantity() >= quantity) {
                ...
                wallet_service.payOrderPayment(order, user);

                Asset updatedAsset=asset_service.updateAsset(
                        assetToSell.getId(),
                        -quantity
                );
                if(updatedAsset.getQuantity()*coin.getCurrentPrice()<=1){
                    asset_service.deleteAsset(updatedAsset.getId());
                }
                return savedOrder;
            }
            else {
